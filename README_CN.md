...
│   ├── indicators/    # 技术指标
│   ├── llm/           # LLM集成
│   ├── viz/           # Streamlit 实验室（builders + 产物浏览）
│   └── utils/         # 配置和工具
├── config/            # 配置文件
...
### 组合 LLM 温度（temperature）

`config.yaml` 中的 `model.temperature`（以及 `benchmark.yaml` 里 `main.model.temperature`）由 `TradingSystemRunner`、`Backtester`、`dag_backtest_runner` 读取，经 `Agent.run(..., model_temperature=...)` 传入 `trading_dag/llm/llm.py` 的 `get_llm()`，作用于**组合节点（PortfolioManagementNode）**所用的 Chat 客户端（OpenAI 兼容、Groq、Anthropic、Gemini、Ollama 等）。策略代理节点当前不使用该字段。一般写 `0.0` 以降低随机性；即便为 `0`，接口仍可能多次调用结果不完全相同。
...
**职责。** `RiskManagementNode`（`nodes/risk.py`）位于策略节点之后、组合节点之前：根据**主周期** OHLC 与当前持仓/现金，为每个标的计算 `suggested_quantity`、`current_price`、`remaining_position_limit` 及代码填充的 `reasoning`。实现为 pandas 与算术，**节点内不调 LLM**。之后的 `PortfolioManagementNode` 在 LLM 或规则路径下决策时须遵守这些数值；走 LLM 时使用配置中的 **`model.temperature`**（见上文「组合 LLM 温度」）。
...
**组合层。** `PortfolioManagementNode` 向 LLM 提供 `current_prices`、`max_shares`（上限 ÷ 价，无效为 0）、`suggested_quantities`；**不**把完整 `reasoning` dict 嵌入 prompt。`summarize_risk_reasoning_for_prompt()` 生成每标的英文 **`risk_context_summary`**。组合 LLM 使用配置中的 **`model.temperature`**（见上文「组合 LLM 温度」）。`show_reasoning` 仅影响控制台。**Ablate_LLMPortfolio** 不调组合 LLM，仍消费上述数值上限。
...
**指标与产物。** 每条权益曲线来自回测器组合价值序列；`trading_dag.benchmark.equity_metrics.build_equity_metrics` 计算收益、Sharpe、Sortino、回撤、胜率等。汇总表按收益排序仅为速览；**正式报告**中仍应单独讨论显著性、成本敏感性、多窗口稳健性等（本 README 不替代统计小节）。Streamlit **Standard backtest → Equity & KPIs** 页眉上的总收益、盈亏、Sharpe、最大回撤由 `viz.helpers._kpis_from_value_series` 调用同一 `build_equity_metrics` 计算，因此当选中的 `backtest_performance_*.csv` 包含与该次运行引擎写出一致的 `portfolio_value` 序列时，与 CLI / benchmark 的 $\sqrt{365}$ 年化及按 bar 摊开的无风险利率口径一致。
