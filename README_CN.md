...
- 自动在`start_date`之前获取额外的历史数据（126个周期）用于技术指标预热
- 每个数据点触发完整的工作流执行：
...
系统自动在`start_date`之前获取126个周期的历史数据，确保：
- 技术指标（RSI、EMA、MACD、布林带等）从第一个数据点就有足够的数据
- 策略可以从回测开始就计算准确的信号
- 不会因数据不足而出现NaN值或默认信号
