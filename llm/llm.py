...
# Try to import JSON parser from langchain_core (new location)
try:
    from langchain_core.output_parsers import JsonOutputParser
    json_parser = JsonOutputParser()
except ImportError:
    # Fallback: try SimpleJsonOutputParser from old location
    try:
        from langchain.output_parsers.json import SimpleJsonOutputParser
        json_parser = SimpleJsonOutputParser()
    except ImportError:
        # Last resort: create a simple JSON parser
...
def get_llm(provider: str, model: str, base_url: Optional[str] = None):
    ...
    """
    print(f"DEBUG: get_llm called with provider={provider}, model={model}, base_url={base_url}")
    timeout = 120  # For ollama can be very slow
    max_retries = 3
