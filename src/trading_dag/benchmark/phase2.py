...
from trading_dag.benchmark.phase1_baselines import prepare_primary_klines
from trading_dag.benchmark.phase2_dag import run_ablation_variant
...
def run_phase2_benchmarks(
    ...
    for spec in ablation_registry:
        experiment_idx += 1
        print(f"\n[Phase2][{experiment_idx}/{total_experiments}] Ablation: {spec.name} - start")
        start_time = perf_counter()
        result, curve = run_ablation_variant(
            spec.name,
            spec.strategies,
            spec.ablation,
            config,
        ...
        print(
            f"[Phase2][{experiment_idx}/{total_experiments}] Ablation: {spec.name} - done "
        ...
        if export_individual_results:
            single_curve_path = out_dir / f"benchmark_phase2_equity_{spec.name}_{timestamp}.csv"
    ...
    if baseline_registry:
        print("\n[Phase2] Preparing baseline market data once for all baseline experiments...")
        ...
        for spec in baseline_registry:
            experiment_idx += 1
            print(f"\n[Phase2][{experiment_idx}/{total_experiments}] Baseline: {spec.name} - start")
            start_time = perf_counter()
            ...
            elapsed = perf_counter() - start_time
            print(
                f"[Phase2][{experiment_idx}/{total_experiments}] Baseline: {spec.name} - done "
    ...
    summary_path = out_dir / f"benchmark_phase2_summary_{timestamp}.csv"
    equity_path = out_dir / f"benchmark_phase2_equity_{timestamp}.csv"
    ...
    if export_individual_results:
        for exp in experiments:
            single_summary_path = out_dir / f"benchmark_phase2_summary_{exp.name}_{timestamp}.csv"
            pd.DataFrame(
                [
                    {
                        "experiment": exp.name,
